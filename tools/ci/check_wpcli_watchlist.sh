...
display_path() {
  ...
  local dir
  dir="$(dirname "$path")"
  printf '%s/%s\n' "$(cd "$(dirname "$dir")" && pwd)/$(basename "$dir")" "$(basename "$path")"
}
...
if (( ${#missing_files[@]} > 0 )); then
  if [[ "$REQUIRE_SIBLING_REPOS" == "1" ]]; then
    for file in "${missing_files[@]}"; do
      echo "Missing canonical source file: $(display_path "$file")"
    done
    exit 1
  fi

  echo "SKIP: WP-CLI regression watchlist requires sibling canonical documents."
